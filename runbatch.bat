...
REM ## It is to trigger the Batch Process of the Database Deployment Pipeline                                 ##
REM ## The Batch Process moves the ETL files and executes the SQL scripts against the database                ##
...
SET APP_DIR=E:\PythonProjects\databaseDeploymentPipelineTool\batchprocess\app
SET LOG_DIR=%APP_DIR%\logs

IF NOT EXIST "%LOG_DIR%" mkdir "%LOG_DIR%"

FOR /F "tokens=1-6 delims=/:. " %%A IN ("%DATE% %TIME%") DO (
    SET LOG_FILE=%LOG_DIR%\batchprocess_%%C%%B%%A_%%D%%E%%F.log
)
...
IF NOT EXIST "%APP_DIR%\batchprocess\app" (
    echo ERROR: Batch process directory not found: %APP_DIR%\batchprocess\app
    pause
    exit /b 1
)
cd /d "%APP_DIR%\batchprocess\app"
...
START "DeploymentPipeline-BatchProcess" /B python -m main >> "%LOG_FILE%" 2>&1
